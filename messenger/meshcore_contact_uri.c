#include "meshcore_contact_uri.h"

#include <stdio.h>
#include <string.h>

#define MESHCORE_URI_PREFIX "meshcore://contact/add?"
...
bool meshcore_contact_uri_parse(const char* uri, mc_contact_t* out) {
    ...
    const size_t prefix_len = strlen(MESHCORE_URI_PREFIX);
    if(strncmp(uri, MESHCORE_URI_PREFIX, prefix_len) != 0) return false;
    ...
    while(*p != '\0') {
        ...
        if(name_len == 10 && strncmp(p, "public_key", 10) == 0) {
            if(val_len == 64 && meshcore_hex_decode(val, 64, key)) have_key = true;
        } else if(name_len == 4 && strncmp(p, "type", 4) == 0) {
            /* Small unsigned integer, digits only. */
            type = 0;
            have_type = val_len > 0;
        ...
        } else if(name_len == 4 && strncmp(p, "name", 4) == 0) {
            meshcore_url_decode(val, val_len, name, sizeof(name));
        }
        /* Unknown keys (e.g. region_scope) are ignored. */
    ...
    if(!have_key || !have_type || type < 1 || type > 4) return false;
